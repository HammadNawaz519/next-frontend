          ...
          <div className="w-full flex flex-col gap-4 animate-in fade-in duration-200">
            ...
            <div className="flex flex-col items-center gap-2 py-2">
              ...
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="text-xs font-bold text-purple-600 hover:text-purple-700 mt-1 cursor-pointer border-0 bg-transparent"
              >
          ...
          <>
            ...
            <div className="flex flex-col items-center text-center pt-1">
              <div className="relative">
                ...
                {/* Online indicator */}
                <div
                  className="absolute bottom-1 left-1 z-20 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white shadow-xs"
                  title="Active"
                />
              </div>
            ...
            {!isSelf && (
              <div className="flex items-center gap-3 w-full">
                <button
                  ...
                  className={`flex-1 py-3.5 px-4 rounded-full font-bold text-sm flex items-center justify-center gap-2 shadow-xs transition-all active:scale-95 cursor-pointer outline-none border-0 ${
                    isFollowing
            ...
            {/* ── ABOUT & DETAILS SECTION (ChatDetails Style) ── */}
            <div className="flex flex-col gap-2">
              ...
              <div className="bg-zinc-50 border border-zinc-100 rounded-[24px] p-4 flex flex-col gap-3 text-sm divide-y divide-zinc-100/80 shadow-2xs">
                ...
                {curEmail && (
                  <div className="flex items-center justify-between pt-3">
                    ...
                    <span className="font-semibold text-zinc-800 flex items-center gap-1.5">
                      <span className="truncate max-w-[180px]">{curEmail}</span>
                ...
                <div className="flex items-center justify-between pt-3">
                  ...
                  {activeUserData?.website ? (
                    <a
                      href={activeUserData.website.startsWith('http') ? activeUserData.website : `https://${activeUserData.website}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-semibold text-purple-600 hover:underline flex items-center gap-1"
                    >
                      <Globe className="w-3.5 h-3.5" />
                      <span className="truncate max-w-[180px]">{activeUserData.website.replace(/^https?:\/\//, '')}</span>
                    </a>
                  ) : (
                    <span className="text-zinc-400 text-xs font-medium">None added</span>
                  )}
                </div>
                ...
                {/* Account Type */}
                <div className="flex items-center justify-between pt-3">
                  <span className="font-medium text-zinc-500">Account Type</span>
                  <span className="font-bold text-zinc-800 flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-purple-600" />
                    <span>Connect Verified</span>
                  </span>
                </div>
              </div>
            </div>

            {/* ── DANGER ZONE & ACCOUNT ACTIONS (Distinct Clean Styling) ── */}
            {isSelf && (
              <div className="flex flex-col gap-2.5 mt-1 pt-2">
                ...
                {/* Sleek Dark Log Out Button (NOT Red) */}
                <button
                  type="button"
                ...
                {/* Designated Red Delete Account Button */}
                <button
                  type="button"
                  onClick={() => {
                    triggerHaptic('medium');
                  ...
                  className="w-full py-3.5 px-6 rounded-full bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/80 font-bold text-sm flex items-center justify-center gap-2.5 transition-all cursor-pointer active:scale-[0.98]"
